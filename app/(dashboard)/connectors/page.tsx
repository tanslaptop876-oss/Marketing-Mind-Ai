...
import { connectWordPress, disconnectGbp, disconnectGsc, disconnectMeta, disconnectWordPress } from "./actions";
...
export default async function Connectors({ searchParams }: { searchParams: Promise<{ message?: string; error?: string }> }) {
  ...
  const [{ data: gscData = [] }, { data: metaData = [] }, { data: wordpressData = [] }, { data: gbpData = [] }] = await Promise.all([
