...
import { disconnectGsc, disconnectMeta } from "./actions";
...
export default async function Connectors({ searchParams }: { searchParams: Promise<{ message?: string; error?: string }> }) {
  ...
  const [{ data: gscData = [] }, { data: metaData = [] }] = await Promise.all([
  ...
  return <>
    ...
    <h2 className="section">Integration roadmap</h2><div className="grid section">{Object.entries(connectorCatalog).filter(([key]) => !["gsc", "meta"].includes(key)).map(([key, connector]) => <div className="card" key={key}>
