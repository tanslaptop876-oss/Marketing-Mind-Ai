...
export default async function Audits({ searchParams }: { searchParams: Promise<{ message?: string; error?: string; website?: string }> }) {
  ...
  return <>
    ...
    <div className="grid section">{[30, 90, 180, 365].map(days => {
      ...
      return <div className="card" key={days}><span className="muted">vs {days} days</span><div className="metric">{delta === null ? "â€”" : `${delta >= 0 ? "+" : ""}${delta}`}</div><span className={`badge ${delta !== null && delta >= 0 ? "good" : delta !== null ? "bad" : ""}`}>{delta === null ? "No baseline" : delta >= 0 ? "Improved" : "Declined"}</span></div>;
    })}</div>
    <div className="grid two section">
      ...
      <div className="card"><h2>Latest result{selectedWebsite ? ` Â· ${selectedWebsite.name}` : ""}</h2>{!latest ? <div className="empty">No audit runs for this website.</div> : <>
        ...
        <div className="grid section"><div><span className="muted small">Technical</span><div><strong>{latest.technical_score ?? "â€”"}</strong></div></div><div><span className="muted small">Content</span><div><strong>{latest.content_score ?? "â€”"}</strong></div></div><div><span className="muted small">Performance</span><div><strong>{latest.performance_score ?? "â€”"}</strong></div></div></div>
